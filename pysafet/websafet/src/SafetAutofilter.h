...
class SafetAutofilter : public SafetXmlObject
{
    ...
    Q_PROPERTY(QString period READ period WRITE setPeriod)
     ...
     QString _id, _side, _source, _type,  _period, _query, _options, _tokenlink,_pattern, _report, _subreport,_recursive;
     ...
     QString _subsql;
     QStringList _subtitutions;

public:
     enum FilterType { None, String, Date, DateTime, Integer, Float};
     enum ForDatePeriod { Hour, Week, Month, Year, Quarter, Day, Price};

     enum DateFormat { ISO, DATESTRING, SECONDS };
private:
     FilterType _filterType;
     SafetTask *createInterTask(const QStringList& mylist, SafetTask* parenttask);
public:
    ...
    void setPeriod(const QString& p) { _period = p; }
    ...
    */
    void modifyForAutofilter(QList<SafetTask*>& lt);
