...
SafetXmlRepository::SafetXmlRepository(QObject *parent)
 : QObject(parent)
{
    _isopencontainer = false;
    _firstsearch = false;
}


SafetXmlRepository::~SafetXmlRepository()
{
}

void SafetXmlRepository::setEnvHome(const QString& home){
	//Q_ASSERT_X(!home.isEmpty(), "SafetDbXmlRepository::setEnvHome()", qPrintable(tr("home is Empty")));
...
QString SafetXmlRepository::getEnvHome() {
	return envHome;
}
