    ...
    SmtpClient smtp(myhost, 25, SmtpClient::TcpConnection);
       ...
       if (plink.indexOf("/activeaccount") != -1 ) {
           ...
           if (mymessage.isEmpty()) {
               mymessage = "Activaci�n de cuenta - Sistema Safet";
           }
           ...
           if (mymessage.isEmpty()) {
               mymessage = "Reinicio de contrase�a - Sistema Safet";
           }

           message.setSubject(mymessage);
           mytext = QString("Para reiniciar la contrase�a del su usuario \"%1\" debe seguir el enlace %2."
                                    "\n\nGracias por su atenci�n.\n\n"
                                    "")
                   .arg(user)
                   .arg(plink);
...
//    QString result = tr("<b>Usuario registrado (%1)</b>. Espere el correo de completaci�n de registro en la cuenta que suministr�.")
//            .arg(account);
...
#define PASSTWO   16
    ...
    if (mymessage.isEmpty()) {
        mymessage = tr("Se enviado un correo de activaci�n de registro.");
    }
    ...
    if ( (typecheck & FULLNAME) == 0) {
        return tr("El campo \"Nombre Completo\" es obligatorio, debe escribir alg�n nombre");
    }
    ...
    if ( ((typecheck & PASSONE) == 0) || ((typecheck & PASSTWO) == 0)) {
        return tr("No puede dejar los campos de asignaci�n de contrase�as vac�os");
    }
    ...
    if (!rx.exactMatch(account) ) {
        return tr("El nombre de la cuenta debe contener caracteres alfanum�ricos, y debe tener una longitud mayor o igual a 3 caracteres");
    }
    ...
    if (!rx.exactMatch(email) ) {
        return tr("El correo electr�nico debe tener una direcci�n v�lida. Por ejemplo: foo@acme.org");
    }

    rx.setPattern(".{6,}");

    if (!rx.exactMatch(passone) ) {
        return tr("La contrase�a debe tener m�s de 6 caracteres");
    }

//    if (passone != passtwo) {
//        return tr("La contrase�as deben ser iguales");
//    }
    ...
    //                .arg(textsha1.toHex().data())
        ...
        QString myaction = "";
            ...
            myaction = QString("operacion:Agregar_usuario Nombre_cuenta_usuario: %1 Contrase�a_usuario: %2 "
...
bool MainWindow::checkAndExecEvents() {
    ...
    while (true)  {
        ...
        if ( ndocs > 0 ) { // Se cumple la condici�n
...
//                "<div id=\"nav\">"
            "<ul class=\"sf-menu dropdown\">"
                ...
                "<li><a href=\"%2\">Gesti�n</a></li>"
                "<li><a href=\"%3\">Consultas</a></li>"
                "<li><a href=\"%4\">Cr�ditos</a></li>"
                "<li><a href=\"%5\">Cerrar Sesi�n</a></li>"
            "</ul>"
//        "</div>"
                )
            .arg(hurl+"/login")
...
QString MainWindow::generateFormHead(const QString& o) {
    ...
    if (purls.count() < 3) {
        SYE
                   <<  tr("Al obtener el formulario la operaci�n \"%1\" es Inv�lida")
                      .arg(purl);
...
/*
        result +=  QString("$( \"#parsdialog\" ).dialog({\n"
                           ...
                           "                  alert(\"Debe Seleccionar un opci�n de flujo de trabajo \\npara ver el Cuadro de Par�metros\");\n"
                           "                  return;\n"
                           "            }\n               "
...
QString MainWindow::generateModifyHTML(const QString& operation, const QString& fieldname,
    ...
    if (operation.startsWith(QLatin1String("Listar_"))
            ...
            || operation.startsWith(QLatin1String("Registrar_completaci�n_de_firmas_"),Qt::CaseInsensitive)
            ...
            || operation.startsWith(QLatin1String("Registrar_aprobaci�n_"),Qt::CaseInsensitive)
            || operation.startsWith(QLatin1String("Registrar_validaci�n_"),Qt::CaseInsensitive)
            || operation.startsWith(QLatin1String("Agregar_planificaci�n"),Qt::CaseInsensitive)
        ...
        if (fieldname == "Periodo_vacacional")  {
            ...
            result = QString("%1")
               ...
               || (operation == "modificar_usuario" && (fieldname == "Pa�s" || fieldname == "Pais"))
               || (operation == "modificar_perfil" && (fieldname == "Pa�s" || fieldname == "Pais"))
               || operation == "agregar_publicaci�n"
               || (operation == "modificar_publicaci�n" && fieldname == "Categor�a")
               || (operation == "modificar_publicaci�n" && fieldname == "SubCategoria")
               || (operation == "modificar_publicaci�n" && fieldname == "SubCategor�a")) {
...
QString MainWindow::generateFormFooter(const QString& o) {
    ...
    if (purl.split(":").count() < 2) {
        SYE                   <<  tr("Al obtener el formulario la operaci�n \"%1\" es Inv�lida")
                      .arg(purl);
        return QLatin1String("");
    }
    ...
    foreach(QString f, fields) {
        ...
        if (mywidget == NULL ) {
            if (trimfield.isEmpty()) {
                SYW  << tr("El nombre del campo est� vacio (buscando informaci�n sobre claves)");
            }
            else {
...
//        SYD    << tr("La operaci�n \"\"%1\"\" no existe en el archivo de autorizaci�n<br/>."
...
//    texto = completingTextEdit->toPlainText().toLatin1();
    ...
    if ( queryForErrors() ) {
            ...
            SYE << tr("Ocurri� un error al procesar la acci�n en el archivo de usuarios");
           return false;
    }
...
QString MainWindow::genTicket(const QString& user) {
    ...
    if (!users.contains(user)) {
        SYW << tr("El usuario \"%1\"para la generaci�n de ticket no se encuentra. No se generar� el ticket")
               .arg(user);
        return result;
    }
    ...
    if (users[user].count() < 4 ) {
        SYW << tr("El usuario \"%1\" no contiene el campo para generaci�n de tickets. No se generar� el ticket")
               .arg(user);
        return result;
    }
...
QString  MainWindow::toInputForm(const QString& action,bool withpermises) {
     ...
     /**
       *
       * pos>=0 indica que es una opci�n de firma electr�nica
       *
       */
     /**
       * IMPLEMENTACI�N DE FUNCIONES, TALES COMO "FIRMAR DOCUMENTO"
       *
       **/
     ...
     /**
       * IMPLEMENTACI�N DE FUNCIONES, TALES COMO "ENVIAR POR CORREO"
       *
       **/
     bool genjson = true;
     ...
     if (xml.indexOf("por_correo") > 0) {
         ...
         if ( results.count() == 0) {
             SYE << tr("No se produjo el resultado esperado de la operaci�n.");
             return QString("");
         ...
         if  (!mydataaction.isEmpty()) {
             ...
             if (!result ) {
                 SYE << tr("No se ejecuto la acci�n para el envio de correo");
                 return QString("");
    ...
    foreach(QString key, data.keys()) {
       ...
       if (rx.exactMatch(key)) {
           ...
           if (pos < mylist.count()) {
               ...
               SYW << tr("La  posici�n \"%1\" del par�metro \"%2\" no es v�lida")
                      .arg(pos)
                      .arg(key);
           }
...
int MainWindow::numberOfDocuments(const QString& t, const QString& idkey,
    ...
    if (mylist.count() < 2 ) {
        ...
        SYW << tr("La etiqueta de DocumentID no se encuentra correctamente escrita en el archivo de configuraci�n safet.conf");
        return false;
    }
    ...
    for (int i=0; i < dcount; i++) {
        ...
        try  {
            ...
            SYW << tr("IMPORTANTE:Ocurri� un error (excepcion) al evaluar el script (two)");
        }
...
QString MainWindow::replaceMarks(const QString& s) {
    ...
    result.replace("Safet::aacute","�");
    result.replace("Safet::eacute","�");
    result.replace("Safet::iacute","�");
    result.replace("Safet::oacute","�");
    result.replace("Safet::uacute","�");
    result.replace("Safet::ntilde","�");
    result.replace("Safet::Ntilde","�");
    return result;
}
...
QString MainWindow::extractParameters(const QString& action) {
    ...
    QRegExp rxpars("(parameters\\.)([a-zA-Z0-9_\\-\\./]+)\\s*:\\s*"
                   "([a-zA-Z_0-9�������'\\*=\\.\\-\\(\\),;%#\\x3c\\x3e\\x2f\\[\\]/\\?\\s]+)");
    QRegExp rxconf("(configurekey\\.)([a-zA-Z0-9_\\-\\./]+)\\s*:\\s*([a-zA-Z_0-9�������'\\*=\\.\\-\\(\\),;%#\\x3c\\x3e\\x2f\\[\\]/\\s\\{\\}:\\?]+)");
...
//            }
		...
		parser.operationName().compare(QLatin1String("Generar_grafico_coloreado"),Qt::CaseSensitive) == 0 
        ...
        else if (parser.operationName().startsWith("Generar_gr�fico_para_clave") ||
                 parser.operationName().startsWith("Generar_gr�fico_de_seguimiento") ||
                parser.operationName().startsWith("Generar_grafico_para_clave")
) {

            SYD << tr("......**MainWindow::toInputConsole....Generar_gr�fico_para_clave.....(1)...");
            ...
            QString texto = QString("-f %1 -p graphviz -g -k %2 ")
                    ...
                    .arg(currid);
                        ...
                        SYD << tr("......**MainWindow::toInputConsole....Generar_gr�fico_para_clave...TEXTO..(1)..texto:|%1|")
                               .arg(texto);
            parseArgs( texto );
            if (! executeParsed() ) {
                return false;
            ...
            if ( true ) {
                ...
                _currentjson = tr("Ocurri� un error al tratar de recuperar pr�ximos estados. Vea el registro para detalles##SAFETMESSAGE##");
            }
            ...
            SYD << tr(".........MainWindow::toInputConsole...(1)...Agregar_planificaci�n");
            ...
            if ( doGeneratePlannedGraph(pathgraph, namegraph,idactivity) ) {
                _currentjson = tr("Se guard� el flujo de trabajo de planificaci�n \"%1\" con �xito##SAFETMESSAGE##")
                        .arg(namegraph);
            }
            else {
            ...
            if (!result.isEmpty()) {
                _currentjson = tr("Se cambi� la nota de la tarea \"%1\" "
            ...
            if (!result.isEmpty()) {
                _currentjson = tr("Se cambi� el rol de la tarea \"%1\" "
                                  "a \"%2\" satisfactoriamente!\n##SAFETMESSAGE##")
                        .arg(taskname)
                        .arg(newrol);
            }
        ...
        else if (parser.operationName().startsWith(tr("Cambiar_informaci�n_del_nodo"))) {

            SYD << tr("\n.....--->MainWindow::toInputConsole....Cambiar_informaci�n_del_nodo........(1)");
            QString namegraph = MainWindow::replaceMarks(data.map["Nombre_grafo"]);
            QString taskname = MainWindow::replaceMarks(data.map["Tarea_a_cambiar"]);
            QString newinfo = MainWindow::replaceMarks(data.map["Informacion_del_nodo"]);

            SYD << tr(".....--->MainWindow::toInputConsole....Cambiar_informaci�n_del_nodo...namegraph:|%1|").arg(namegraph);
            SYD << tr(".....--->MainWindow::toInputConsole....Cambiar_informaci�n_del_nodo...taskname:|%1|").arg(taskname);
            SYD << tr(".....--->MainWindow::toInputConsole....Cambiar_informaci�n_del_nodo...newinfo:|%1|").arg(newinfo);
            ...
            SYD << tr(".....--->MainWindow::toInputConsole....Cambiar_informaci�n_del_nodo...result:|%1|").arg(result);
...
void MainWindow::proccessConfFile(const QString& sql, const QString& filename, bool multiplefields) {
    ...
    SYD << tr(".....MainWindow::proccessConfFile...secondkeyfield: |%1|").arg(secondkeyfield);
     ...
     QString searchtext = QString("\\s*(%1)\\s*\\=\\s*([�������a-zA-Z0-9\\*\\$\\#/\\-\\._/:!\\?\\^\\$\\(\\)#%\\x3c\\x3e\\x2f@]"
...
bool MainWindow::doSaveGraph(const QStringList& mypars) {
   ...
   if ( mypars.count() < 4 ) {
       SYE << tr("El n�mero de parametros a guardar debe ser cuatro (4): nombre, codigo, fecha + ruta del flujo y clave");
       return false;
   }
...
// { \"Nombre\": \"%1\", \"Entrega\": \"%2\", \"Planeada\": \"%3\",\"Info\": \"%4\" %5},\n")
    ...
    if ( currentnode.isEmpty()) {
        SYW << trUtf8("NO se encuentra el nodo donde se encuentra la clave para el gr�fico a comparar");
        currentnode = "inicial";
    }
    ...
    bool uncomplete = false;
        ...
        if ( !issecondfound ) {
            SYW << tr(" Generando comparaci�n de grafos: no se encontr� el \"%1\"")
...
//    buffer[3] = 0;
   ...
   //     executeJSCodeAfterLoad();
        SYD << tr("...despu�s de ejecutar: executeJSCodeAfterLoad");
...
void MainWindow::successVerification(QStringList list, const QString& msg, SafetDocument doc){
    ...
    QString message;
    if ( msg.isEmpty()) {
        message = QString("%1").arg(tr("Verificaci�n exitosa"));
    }
    else {
...
bool MainWindow::login(const QString& name, const QString& pass) {
    ...
    if ( users[curuser].at(0) != QString(hashvalue.toHex().data()) && !isticket) {
        SYE  <<
            trUtf8("La contrase�a o usuario \"%1\" no son correctos para ingresar al sistema.");
        SYD << tr("....MainWindow::login....contrase�a incorrecta");
        return false;
    }
...
void MainWindow::doSaveConfiguration(const QString& fileName) {
   ...
   SYD << tr(".........MainWindow::doSaveConfiguration.................(outside)....");
    ...
    if(fileName.isEmpty()) {
        SYE << tr("El nombre de archivo \"%1\" colocado como par�metro no puede ser vac�o")
               .arg(fileName);
        return;
    }
    QStringList myfiles;
