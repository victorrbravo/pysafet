...
YAHOO.lang.extend(WireIt.Wire, WireIt.CanvasElement, {
   ...
   redraw: function() {
	 ...
	 * Position the label element to the center
         * Corrección hecha por José Ruiz 07/10/11 se agregó al final de los sets   +"px"
	 */
      ...
      var obj = {
		...
		if(this.labelEditor) {
			obj.label = this.labelField.getValue();
                        //Agegado por José Ruiz 07/10/11 para que exporte estos parametros
                        obj.labelStyle = this.labelStyle;
                        obj.labelEditor = this.labelEditor;
		}
...
YAHOO.lang.extend(WireIt.FormContainer, WireIt.Container, {
   ...
   renderForm: function() {
      ...
      /**
       *  Añadida la asignación de la opción collapsed por José Ruiz  28/03/11
       */
      var groupParams = {parentEl: this.bodyEl, fields: this.fields, legend: this.legend, collapsible: this.collapsible, collapsed: this.collapsed};
...
inputEx.messages.emptyInPlaceEdit = "(click to edit title)";
	 ...
	 getPipeByName: function(name) {
	    ...
	    for(var i = 0 ; i < n ; i++) {
	       if(this.pipes[i].name == name) {
		       ...
		       /********* Añadido por José Ruiz: 15-03-11 ********
		        * Corrigiendo falla de versión beta para poder
		        * cargar los datos guardados con IndexedDB
		        ***************************************************/
